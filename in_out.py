    ...
    while True:
        ...
        if right == "" and left == "":
            if x > 500:
                ...
                if x < 200:
                    cv2.putText(frame1, "To Left", (10,80), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,255,0), 2)
                    x = 300
                    right, left = "", ""
                    save_loc = os.path.join("visitors", "in")
                    utils.save_frame_with_timestamp(frame1, save_location=save_loc)
        elif left:
                if x > 500:
                    cv2.putText(frame1, "To Right", (10,80), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,255,0), 2)
                    x = 300
                    right, left = "", ""
                    save_loc = os.path.join("visitors", "out")
                    utils.save_frame_with_timestamp(frame1, save_location=save_loc)
